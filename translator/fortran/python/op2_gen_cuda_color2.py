...
def op2_gen_cuda_color2(master, date, consts, kernels, hydra, bookleaf, utblock):
...
##########################################################################
    if hydra:
      ...
      if not os.path.exists(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/CUDA')):
        os.makedirs(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/CUDA'))      
      fid = open(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/CUDA')+name+'_cudakernel.CUF','w')         
    else:
...
##########################################################################
#  auto-gen kernel names for Makefile 
##########################################################################
    if utblock: 
      if not os.path.exists('Makefile_cudakernel_list.txt'):
        os.mknod('Makefile_cudakernel_list.txt')
      if nk==0: 
        fid_make = open('Makefile_cudakernel_list.txt','w')
        fid_make.write('utblock_cuda: directories \\\n')  
        ...
        fid_make.write('\t'+'./Common/main_constants.F90 \\\n')  
        fid_make.write('\t'+'utilities.F90 \\\n')         
        fid_make.write('\t'+'global_gas_functions.F90 \\\n') 
        fid_make.write('\t'+'./Common/IO.F90 \\\n') 
        fid_make.write('\t'+'./Configuration-Files/cudaConfigurationParams.F95 \\\n')           
        prefix_i_pass = []                    
      fid_make.write('\t'+prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/CUDA')+name+'_cudakernel.CUF \\\n')
...
def makefile_create(kernels, fid_make, prefixes, prefix_i_pass):
  ...
  fid_make.write('\t'+'$(FC) $(FFLAGS) $(OPT_CUDA) $(FMODS)/cuda $(FMODS) -c $(FLINK) -lop2_for_cuda -lop2_hdf5 $(HDF5_LIB) $(MPI_LIB) \\\n')
  for nk in range (0,len(kernels)):
    name  = kernels[nk]['name']
    if nk==0: 
      fid_make.write('\t'+'./Configuration-Files/cudaConfigurationParams.F95 \\\n')             
      ...
      fid_make.write('\t'+'global_gas_functions.F90 \\\n') 
      fid_make.write('\t'+'./Common/IO.F90 \\\n')                         
    fid_make.write('\t'+prefixes[prefix_i_pass[nk]].replace('User-Kernels','Auto-Kernels/CUDA')+name+'_cudakernel.CUF \\\n')    

  fid_make.write('\t \t'+'external_subprograms_op.F90 \\\n')  
  fid_make.write('\t \t'+'utblock_op.F90 \n')   
  fid_make.write('\t'+'$(FC) $(FFLAGS) $(OPT_CUDA) $(FMODS)/cuda $(FMODS) \\\n')
  for nk in range (0,len(kernels)):
    name  = kernels[nk]['name']
    if nk==0: 
      fid_make.write('\t'+'cudaConfigurationParams.o \\\n')  
      fid_make.write('\t'+'kdtree2.o \\\n')         
      fid_make.write('\t'+'psuedo_laplacian_cellnode.o \\\n')        
      fid_make.write('\t'+'constants.o \\\n')        
      fid_make.write('\t'+'main_constants.o \\\n')  
      fid_make.write('\t'+'utilities.o \\\n')       
      fid_make.write('\t'+'global_gas_functions.o \\\n') 
      fid_make.write('\t'+'IO.o \\\n')     
    fid_make.write('\t'+name+'_cudakernel.o \\\n')          
  fid_make.write('\t'+'external_subprograms_op.o  utblock_op.o -o utblock_cuda \\\n')     
  fid_make.write('\t'+'$(FLINK) $(FFLAGS) -lop2_for_cuda -lop2_hdf5  $(HDF5_LIB) $(MPI_LIB) \n')        
