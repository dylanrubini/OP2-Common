...
def op2_gen_mpiseq3(master, date, consts, kernels, hydra, bookleaf, utblock):
...
##########################################################################
    if hydra:
      ...
      if not os.path.exists(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/MPI_SEQ3')):
        os.makedirs(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/MPI_SEQ3'))      
      fid = open(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/MPI_SEQ3')+name+'_seqkernel.F90','w')      
    else:
      fid = open(name+'_seqkernel.F90','w')
...
##########################################################################
    if utblock: 
      if not os.path.exists('Makefile_seqkernel_list.txt'):
        os.mknod('Makefile_seqkernel_list.txt')
      if nk==0: 
        fid_make = open('Makefile_seqkernel_list.txt','w')
        fid_make.write('utblock_mpi_genseq: directories \\\n')  
        ...
        prefix_i_pass = []                    
      fid_make.write('\t'+prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/MPI_SEQ3')+name+'_seqkernel.F90 \\\n')
...
def makefile_create(kernels, fid_make, prefixes, prefix_i_pass):
  ...
  fid_make.write('\t'+'$(FC) $(FFLAGS) $(OPT) $(OPENMP)  $(FMPI_MODS) $(MPI_INC) $(HDF5_INC) -c \\\n')
  for nk in range (0,len(kernels)):
    name  = kernels[nk]['name']
    if nk==0: 
      fid_make.write('\t'+'./kdtree2/src-f90/kdtree2.f90 \\\n')     
  ...
  fid_make.write('\t \t'+'external_subprograms_op.F90 \\\n')  
  fid_make.write('\t \t'+'utblock_op.F90 \n')   
  fid_make.write('\t'+'$(FC) $(FFLAGS) $(OPT) $(OPENMP)  $(PARMETIS_INC) $(PTSCOTCH_INC) $(HDF5_INC) \\\n')
  for nk in range (0,len(kernels)):
    name  = kernels[nk]['name']
    if nk==0: 
  ...
  fid_make.write('\t'+'$(FLINK) $(FFLAGS) -lop2_for_mpi $(HDF5_LIB) $(PARMETIS_LIB) $(PTSCOTCH_LIB) $(MPI_LIB) $(CPPLINK)\n')        
