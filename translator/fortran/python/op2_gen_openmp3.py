...
def op2_gen_openmp3(master, date, consts, kernels, hydra,bookleaf, utblock):
...
##########################################################################
    if hydra:
      ...
      if not os.path.exists(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/OpenMP3')):
        os.makedirs(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/OpenMP3'))      
      fid = open(prefixes[prefix_i].replace('User-Kernels','Auto-Kernels/OpenMP3')+name+'_openmp3.F90','w')          
    else:
...
##########################################################################
    if utblock: 
      if not os.path.exists('Makefile_openmp3_list.txt'):
...
def makefile_create(kernels, fid_make, prefixes, prefix_i_pass):
  ...
  fid_make.write('\t'+'$(MPIF90) $(FFLAGS) $(OPT) $(OPENMP) $(FMODS) $(MPI_INC) $(HDF5_INC) -c \\\n')
  for nk in range (0,len(kernels)):
    name  = kernels[nk]['name']
    if nk==0: 
      fid_make.write('\t'+'./kdtree2/src-f90/kdtree2.f90 \\\n')     
  ...
  fid_make.write('\t \t'+'external_subprograms_op.F90 \\\n')  
  fid_make.write('\t \t'+'utblock_op.F90 \n')   
  fid_make.write('\t'+'$(MPIF90) $(FFLAGS) $(OPT) $(OPENMP)  $(HDF5_INC) \\\n')
  for nk in range (0,len(kernels)):
    name  = kernels[nk]['name']
    if nk==0: 
  ...
  fid_make.write('\t'+'$(FLINK) $(FFLAGS) -lop2_for_openmp4 -lop2_hdf5 $(HDF5_LIB) $(MPI_LIB) \n')        
